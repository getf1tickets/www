...
export default function ProductDetailsCarousel({ product }) {
  ...
  const handleOpenLightbox = (url) => {
    const selected = imagesLightbox.findIndex((index) => index === url);
  ...
  return (
    <RootStyle>
      <Box sx={{ p: 1 }}>
        <Box sx={{
          zIndex: 0, borderRadius: 2, overflow: 'hidden', position: 'relative',
        }}
        >
        ...
        sx={{
          ...
          ...(product.images.length > 2 && {
            ...
            '&:before, &:after': {
              ...
              backgroundImage: (theme) => `linear-gradient(to left, ${alpha(theme.palette.background.paper, 0)} 0%, ${
